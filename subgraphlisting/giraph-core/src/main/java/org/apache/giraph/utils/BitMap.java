...
//        	listCount++;
	...
	public boolean intersection(BitMap bm){
		 int indexLimit = wordInUse > bm.wordInUse ? bm.wordInUse : wordInUse;
	     // Perform logical AND on words in common
	     for (int i = 0; i < indexLimit; i++)
	    	 if(Long.bitCount(words[i] & bm.words[i]) > 0)
	    		 return true;
	     return false;
	}
...
//            sizeIsSticky = false;
	...
	public boolean cover(BitMap bm) {
		 int indexLimit = wordInUse > bm.wordInUse ? bm.wordInUse : wordInUse;
	     // Perform logical AND on words in common
	     for (int i = 0; i < indexLimit; i++){
	    	 if((words[i] & bm.words[i]) != bm.words[i])
	    		 return false;
	     }
	     if(bm.wordInUse > wordInUse){
	    	 for(int i = indexLimit; i < bm.wordInUse; i++){
	    		 if(bm.words[i] != 0)
	    			 return false;
	    	 }
	     }
