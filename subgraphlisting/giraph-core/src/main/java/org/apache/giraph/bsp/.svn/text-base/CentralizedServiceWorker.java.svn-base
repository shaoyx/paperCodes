...
public interface CentralizedServiceWorker<I extends WritableComparable,
  ...
  WorkerGraphPartitioner<I, V, E, M> getWorkerGraphPartitioner();
  
  void addLocalMessage(WritableRequest writableRequest);

HashMap<Integer, VertexProperty> getVertex2PropertyMap();

QueryGraph getQueryGraph();

QueryGraphSequence getQueryGraphSequence();

void incMatchedSubgraphCount();

void incMatchedUncompleteCount();

long getMatchedSubgraphCount();

long getMatchedUncompleteCount();

void incMatchedSubgraphCount(long count);

BloomFilter getGraphEdgeIndex();
}
