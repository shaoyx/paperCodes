...
import org.apache.giraph.utils.QueryGraph;
import org.apache.giraph.utils.QueryGraphSequence;
...
public abstract class Vertex<I extends WritableComparable,
  ...
  private VertexEdges<I, E> bigEdges; // target id larger than id;
  private VertexEdges<I, E> smallEdges; // target id larger than id;
  private HashSet<Integer> edgeTargetIdSet; /* PSgL related structure. */
  ...
  private long invalidByMatchedVertex; //valid 1
  private long invalidByMatchedVertexIndex; //valid 1
  private long invalidByUnmatchedVertex; //valid 2
  private long timeCostPerPartition;
//	private final AtomicInteger msgCount = new AtomicInteger();
//	private final AtomicInteger sendMsgCount = new AtomicInteger();
//	private final AtomicInteger invalidByMatchedVertex = new AtomicInteger(); //valid 1
//	private final AtomicInteger invalidByUnmatchedVertex = new AtomicInteger(); //valid 2
private long checkCandidateNeighborhoodCost;
private long getCandidateCost;
private long checkOrientationCost;
private long checkValidCost;
private long distributorCost;
private long invalidByOrientation;
...
protected Distributor distributor;
  ...
  public void initialize(I id, V value, Iterable<Edge<I, E>> edges) {
    this.id = id;
    this.value = value;
    edgeTargetIdSet = new HashSet<Integer>();
    setEdges(edges);
  }
...
//    System.out.println("Call initialization with parameter");
    this.bigEdges = conf.createAndInitializeVertexEdges(0);
    this.smallEdges = conf.createAndInitializeVertexEdges(0);
  }
  ...
  public void setEdges(Iterable<Edge<I, E>> edges) {
    ...
    /* separate big and small edges */
    Iterator<Edge<I, E>> edgeIter = this.edges.iterator();
    while(edgeIter.hasNext()){
  	  Edge<I,E> edge = edgeIter.next();
  	  edgeTargetIdSet.add(edge.getTargetVertexId().hashCode());
  	 if(edge.getTargetVertexId().compareTo(this.id) < 0){
  		 this.bigEdges.add(edge);
  	 }
  	 else{
  		 this.smallEdges.add(edge);
  	 }
    }

//	  System.out.println("construct in initialization: currentId="+getId().hashCode()+": ");
//	  for(int temp : edgeTargetIdSet){
...
//	  System.out.println();
  ...
  public Iterable<Edge<I, E>> getBigEdges() {
	return bigEdges;
  }
  
  public Iterable<Edge<I, E>> getSmallEdges() {
	return smallEdges;
  }
...
//	  MemoryUtils.getRuntimeMemoryStats();
  ...
  public void readFields(DataInput in) throws IOException {
    ...
    bigEdges = conf.createVertexEdges();
    bigEdges.readFields(in);
    
    smallEdges = conf.createVertexEdges();
    smallEdges.readFields(in);
    ...
    Iterator<Edge<I, E>> edgeIter = this.edges.iterator();
    while(edgeIter.hasNext()){
  	  edgeTargetIdSet.add(edgeIter.next().getTargetVertexId().hashCode());
    }

//	  System.out.println("construting from remote: currentId="+getId().hashCode()+": ");
//	  for(int temp : edgeTargetIdSet){
//		  System.out.print(temp+" ");
//	  }
//	  System.out.println();
  ...
  /**
   * Interface to change vertexId, and it is only used for
   * IdenticalWithRelabelWorkerPartitioner
   * @param id
   */
...
//	  if(id instanceof I)
  ...
  /***************** subgraph match related *********************/
  /**
   * Check the targetId existence
   */
  public boolean isTargetIdExist(int tid){
//	  edgeTargetIdSet.
//	  System.out.println("currentId="+getId().hashCode()+": ");
//	  for(int temp : edgeTargetIdSet){
//		  System.out.print(temp+" ");
//	  }
//	  System.out.println();
	  return edgeTargetIdSet.contains(tid);
  }
  
  /**
   * get vertex2Label map
   * @return
   */
  public HashMap<Integer, VertexProperty> getVertex2PropertyMap(){
	  return graphState.getGraphTaskManager().getVertex2PropertyMap();
  }
  
  public BloomFilter getEdgeIndex(){
	  return graphState.getGraphTaskManager().getGraphEdgeIndex();
  }
  ...
  public QueryGraph getQueryGraph(){
	  return graphState.getGraphTaskManager().getQueryGraph();
  }
  
  public QueryGraphSequence getQueryGraphSequence(){
	  return graphState.getGraphTaskManager().getQueryGraphSequence();
  }
  
  /**
   * 
   */
  public void incMatchedSubgraphCount(){
	  graphState.getGraphTaskManager().incMatchedSubgraphCount();
  }
  
  public void incMatchedUncompleteCount(){
	  graphState.getGraphTaskManager().incMatchedUncompleteCount();
  }
  
  public void incMatchedSubgraphCount(long count) {
	  graphState.getGraphTaskManager().incMatchedSubgraphCount(count);
  }
  
  public int getInitLabel(){
	  return this.conf.getInt(GiraphConstants.GIRAPH_SUBGRAPHMATCH_INITLABEL, GiraphConstants.DEFAULT_GIRAPH_SUBGRAPHMATCH_INITLABEL);
  }
  
  public int getInitPatternVertex(){
	  return this.conf.getInt(GiraphConstants.GIRAPH_SUBGRAPHMATCH_INITPV, 
			  GiraphConstants.DEFAULT_GIRAPH_SUBGRAPHMATCH_INITPV);
  }
  
  /* tracking message processed by this vertex. */
  ...
  public void resetSendMessageCount(){
	  sendMsgCount = 0;
  }
  
  public void incInvalidByMatchedVertex(){
	  invalidByMatchedVertex++;
  }
  
  public long getInvalidByMatchedVertex(){
	  return invalidByMatchedVertex;
  }
  
  public void resetInvalidByMatchedVertex(){
	  invalidByMatchedVertex = 0;
  }
  
  public void incInvalidByMatchedVertexIndex(){
	  invalidByMatchedVertexIndex++;
  }
  
  public long getInvalidByMatchedVertexIndex(){
	  return invalidByMatchedVertexIndex;
  }
  
  public void resetInvalidByOrientation(){
	  invalidByOrientation = 0;
  }
  
  public void incInvalidByOrientation(){
	  invalidByOrientation++;
  }
  
  public long getInvalidByOrientation(){
	  return invalidByOrientation;
  }
  
  
  public void resetInvalidByMatchedVertexIndex(){
	  invalidByMatchedVertexIndex = 0;
  }
  
  
  public void incInvalidByUnmatchedVertex(){
	  invalidByUnmatchedVertex++;
  }
  
  public long getInvalidByUnmatchedVertex(){
	  return invalidByUnmatchedVertex;
  }
  
  public void resetInvalidByUnmatchedVertex(){
	  invalidByUnmatchedVertex = 0;
  }
  
  public void incTimeCost(long timeMilliCost){
	timeCostPerPartition += timeMilliCost;  
  }
  
  public long getTimeCost(){
	  return timeCostPerPartition;
  }
  
  public void resetTimeCost(){
	  timeCostPerPartition = 0;
  }
  
  public void incCheckOrientationCost(long timeMilliCost){
	checkOrientationCost += timeMilliCost;  
  }
  
  public long getCheckOrientationCost(){
	  return checkOrientationCost;
  }
  
  public void resetCheckOrientationCost(){
	  checkOrientationCost = 0;
  }
  
  public void incGetCandidateCost(long timeMilliCost){
	getCandidateCost += timeMilliCost;  
  }
  
  public long getGetCandidateCost(){
	  return getCandidateCost;
  }
  
  public void resetGetCandidateCost(){
	  getCandidateCost = 0;
  }
  
  public void incCheckCandidateNeighborhoodCost(long timeMilliCost){
	checkCandidateNeighborhoodCost += timeMilliCost;  
  }
  
  public long getCheckCandidateNeighborhoodCost(){
	  return checkCandidateNeighborhoodCost;
  }
  
  public void resetCheckCandidateNeighborhoodCost(){
	  checkCandidateNeighborhoodCost = 0;
  }
  
  public void incCheckValidCost(long timeMilliCost){
	checkValidCost += timeMilliCost;  
  }
  
  public long getCheckValidCost(){
	  return checkValidCost;
  }
  
  public void resetCheckValidCost(){
	  checkValidCost = 0;
  }
  ...
  public void incDistributorTime(long timeMilliCost){
	  distributorCost += timeMilliCost;
  }
  ...
  public void resetDistributorTime(){
	  distributorCost = 0;
  }
}
