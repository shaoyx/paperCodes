...
//    		        this);
//      }
//      else if("page".equals(this.conf.get(GiraphConstants.GIRAPH_ENGINE,GiraphConstants.DEFAULT_GIRAPH_ENGINE))){
//          if (LOG.isInfoEnabled()) {
//              LOG.info("instantiate page Engine!");
//            }
//    	  serviceWorker = new BspServiceWorkerPage<I, V, E, M>(
//    		        serverPortList,
//    		        sessionMsecTimeout,
//    		        context,
//    		        this);
//      }
//      else 
      if("default".equals(this.conf.get(GiraphConstants.GIRAPH_ENGINE,GiraphConstants.DEFAULT_GIRAPH_ENGINE))){
          if (LOG.isInfoEnabled()) {
  ...
  private void processGraphPartitions(Mapper<?, ?, ?, ?>.Context context,
    ...
    /**
     * The BlockingQueue indicates that each thread only process
     * one partition.
     */
...
//	  if(serviceWorker instanceof BspServiceWorker){
		  return this.serviceWorker.getVertex2PropertyMap();
//	  }
//	  return null;
  }

public QueryGraph getQueryGraph() {
	return this.serviceWorker.getQueryGraph();
}

public QueryGraphSequence getQueryGraphSequence() {
	return this.serviceWorker.getQueryGraphSequence();
}

public void incMatchedSubgraphCount(){
	this.serviceWorker.incMatchedSubgraphCount();
}

public void incMatchedUncompleteCount(){
	this.serviceWorker.incMatchedUncompleteCount();
}

public void incMatchedSubgraphCount(long count) {
	this.serviceWorker.incMatchedSubgraphCount(count);
	
}

public BloomFilter getGraphEdgeIndex() {
	return serviceWorker.getGraphEdgeIndex();
}
