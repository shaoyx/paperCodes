...
public class NettyWorkerServerWithBlockQueueThread<I extends WritableComparable,
    V extends Writable, E extends Writable, M extends Writable>
    implements WorkerServer<I, V, E, M> {
  /** Class logger */
  private static final Logger LOG =
    Logger.getLogger(NettyWorkerServerWithBlockQueueThread.class);
  ...
  private int incomingMessagesReceiverSize;
  private final List<IncomingMessagesReceiver> incomingMessageRecievers = new ArrayList<IncomingMessagesReceiver>();
  /**
   ...
   */
  public NettyWorkerServerWithBlockQueueThread(ImmutableClassesGiraphConfiguration<I, V, E, M> conf,
    ...
    /**
     * Similar Logical to PageWithQueue in handling local messages.
     */
    incomingMessagesReceiverSize = conf.getInt(GiraphConstants.GIRAPH_AYSNCLOCAL_CONCURRENCY, GiraphConstants.DEFAULT_GIRAPH_AYSNCLOCAL_CONCURRENCY);
    LOG.info("The concurrency of incomingMessageReceiver is "+incomingMessagesReceiverSize);
    for(int i = 0; i < incomingMessagesReceiverSize; i++){
    	incomingMessageRecievers.add(new IncomingMessagesReceiver(recievedMessagesForNextSuperstepBuffer, serverData));
    	incomingMessageRecievers.get(i).start();
    }
    nettyServer.start();
  }
  ...
  public void close() {
    nettyServer.stop();
    for(int i = 0; i < incomingMessagesReceiverSize; i++){
    	while(incomingMessageRecievers.get(i).hasFinished()){
//    		LOG.info("Stop incomingMessageReciever before");
    		incomingMessageRecievers.get(i).interrupt();
//    		LOG.info("Stop incomingMessageReciever after");
  ...
  public boolean isFinishedProcessingRecievedMessage(int idx){
	  return this.incomingMessageRecievers.get(idx).hasFinished();
  }
  
  public int getIncomingMessageRecieverSize(){
	  return this.incomingMessagesReceiverSize;
  }
