...
public final class ConfigurationUtils {
  ...
  static {
    ...
    OPTIONS.addOption("qp", "querygraph path", true, "QueryGraph path");
    OPTIONS.addOption("qeo", "querygraph edge orientation path", true, "QueryGraph Edge Orientation Path");
    OPTIONS.addOption("qt", "querygraph type", true, "QueryGraph type");
    OPTIONS.addOption("vl", "vertex2label map path", true, "vertex2label map path");
    OPTIONS.addOption("ei", "edge index path", true, "edge index path");
    OPTIONS.addOption("il", "initial label", true, "initial label");
    OPTIONS.addOption("wpc", "GraphPartitionerFactoryClass", true, "Graph Partitioner Factory class");
  ...
  private static void populateGiraphConfiguration(final GiraphConfiguration
    ...
    if(cmd.hasOption("wpc")){
    	giraphConfiguration.setGraphPartitionerFactoryClass(
    			(Class<? extends GraphPartitionerFactory>)
    			Class.forName(cmd.getOptionValue("wpc")));
    }
    ...
    /* parameter for subgraph match */
    if(cmd.hasOption("qp")){
    	giraphConfiguration.set(GiraphConstants.GIRAPH_SUBGRAPHMATCH_QUERYGRAPHPATH, 
    			cmd.getOptionValue("qp"));
    }
    if(cmd.hasOption("qeo")){
    	giraphConfiguration.set(GiraphConstants.GIRAPH_SUBGRAPHMATCH_QUERYGRAPH_EDGEORIENTATION, 
    			cmd.getOptionValue("qeo"));
    }
    if(cmd.hasOption("qt")){
    	giraphConfiguration.set(GiraphConstants.GIRAPH_SUBGRAPHMATCH_QUERYGRAPH_TYPE, 
    			cmd.getOptionValue("qt"));
    }
    if(cmd.hasOption("vl")){
    	giraphConfiguration.set(GiraphConstants.GIRAPH_SUBGRAPHMATCH_VERTEXTOLABELMAP,
    			cmd.getOptionValue("vl"));
    }    
    if(cmd.hasOption("ei")){
    	giraphConfiguration.set(GiraphConstants.GIRAPH_SUBGRAPHMATCH_EDGEINDEX,
    			cmd.getOptionValue("ei"));
    }
    if(cmd.hasOption("il")){
    	giraphConfiguration.set(GiraphConstants.GIRAPH_SUBGRAPHMATCH_INITLABEL,
    			cmd.getOptionValue("il"));
    }
//    System.out.println("VertexClassName="+vertexClassName+", "+vertexClassName.contains("GeneralQuery"));
    if(vertexClassName.contains("GeneralQuery")){
    	giraphConfiguration.set(GiraphConstants.GIRAPH_SUBGRAPHMATCH_QUERYGRAPH_TYPE, 
    			"general");
    }
  }
