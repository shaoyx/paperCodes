...
import org.apache.giraph.comm.requests.SendWorkerMessagesRequestFactory;
...
public class NettyWorkerClientRequestProcessor<I extends WritableComparable,
  ...
  private boolean asyncLocal;
  private boolean isOpen;
  
  /*customized metrics*/
  private long localRequestCost = 0;
  private long remoteRequestCost = 0;
	private int localRequestCounter;
	private int remoteRequestCounter;
	private long bytecount;
	private long localbytecount;
	
	private final SendWorkerMessagesRequestFactory<I,M> sendWorkerMessagesRequestFactory;
  ...
  public NettyWorkerClientRequestProcessor(
    ...
    this.asyncLocal = false;
   if("asynclocal".equals(configuration.get(GiraphConstants.GIRAPH_ENGINE, GiraphConstants.DEFAULT_GIRAPH_ENGINE)) ||
		   "pagewithqueue".equals(configuration.get(GiraphConstants.GIRAPH_ENGINE, GiraphConstants.DEFAULT_GIRAPH_ENGINE)) ||
		   "page".equals(configuration.get(GiraphConstants.GIRAPH_ENGINE, GiraphConstants.DEFAULT_GIRAPH_ENGINE)) ) 
	   this.asyncLocal = true;
   this.isOpen = false;
    this.serviceWorker = serviceWorker;
    this.serverData = serviceWorker.getServerData();
    
    this.sendWorkerMessagesRequestFactory = new SendWorkerMessagesRequestFactory();
    this.sendWorkerMessagesRequestFactory.setRequestType(configuration);
...
//    /*short-cut for local message*/
//    if(serviceWorker.getWorkerInfo().getTaskId() ==
//         workerInfo.getTaskId()){
//    	 long start_time = System.currentTimeMillis();
//    	 ByteArrayVertexIdMessages<I, M> bmsg =  new ByteArrayVertexIdMessages<I, M>();
//         bmsg.setConf(configuration);
//         bmsg.initialize(16);
//    	 bmsg.add(destVertexId, message);
//    	 try{
//    		 serverData.getIncomingMessageStore().addPartitionMessages(partitionId, bmsg);
//    	 }catch (Exception e)
//    	 {
//    		 
//    	 }
//        this.localRequestCost += System.currentTimeMillis() - start_time;
//        localRequestCounter++;
//    	return true;
//    }
    ...
    if (workerMessageSize >= maxMessagesSizePerWorker) {
      ...
      WritableRequest writableRequest =
    		  sendWorkerMessagesRequestFactory.createSendWorkerMessagesRequest(configuration, workerMessages);
//          new SendWorkerMessagesRequest<I, M>(workerMessages);
  ...
  public void flush() throws IOException {
    ...
    while (iterator.hasNext()) {
      iterator.next();
      WritableRequest writableRequest =
    		  sendWorkerMessagesRequestFactory.createSendWorkerMessagesRequest(configuration,iterator.getCurrentSecond());
...
//              iterator.getCurrentSecond());
  ...
  private void doRequest(WorkerInfo workerInfo,
    ...
    if (serviceWorker.getWorkerInfo().getTaskId() ==
        workerInfo.getTaskId()) {
    	if(this.asyncLocal && this.isOpen){
    		/* TODO: add async. Interface in workerClient */
    		serviceWorker.addLocalMessage(writableRequest);
    	}
    	else{
      ...
      localRequests.inc();
      localRequestCounter++;
      if(writableRequest instanceof SendWorkerMessagesRequest){
    	  localbytecount  += ((SendWorkerMessagesRequest)writableRequest).getSerializedSize();
      }
    } else {
      workerClient.sendWritableRequest(
          workerInfo.getTaskId(), writableRequest);
      this.remoteRequestCost  += System.currentTimeMillis() - start_time;
      remoteRequests.inc();
      remoteRequestCounter++;
      if(writableRequest instanceof SendWorkerMessagesRequest){
    	  bytecount  += ((SendWorkerMessagesRequest)writableRequest).getSerializedSize();
      }
    }
  }

	@Override
	public long getLocalRequestCost() {
		  long result = this.localRequestCost;
		  this.localRequestCost = 0;
		  return result;
	}
	
	@Override
	public long getRemoteRequestCost() {
		  long result = this.remoteRequestCost;
		  this.remoteRequestCost = 0;
		  return result;
	}
	
	@Override
	public int getLocalCounter(){
		int result = this.localRequestCounter;
		this.localRequestCounter = 0;
		return result;
	}
	
	@Override
	public int getRemoteCounter(){
		int result = this.remoteRequestCounter;
		this.remoteRequestCounter = 0;
		return result;
	}
	
	@Override
	public long getRemoteByteCounter(){
	...
	@Override
	public long getLocalByteCounter(){
		long result = localbytecount;
		localbytecount = 0;
		return result;
	}
	...
	public long getMatchedSubgraphCount() {
		return serviceWorker.getMatchedSubgraphCount();
	}
}
