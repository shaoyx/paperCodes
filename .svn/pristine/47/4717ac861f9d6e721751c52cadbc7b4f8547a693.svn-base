...
public interface CentralizedServiceWorker<I extends WritableComparable,
  ...
  boolean isSamePartition(int vid1, int vid2);

  void addLocalMessage(WritableRequest writableRequest);

  /**
   * API for accessing query plan
   * @return
   */
  ArrayList<QueryNode> getQueries();
  boolean isLeftQuery(int qid);
  QueryNode getQueryByChild(int qid);
  int getPlanSize();
}
