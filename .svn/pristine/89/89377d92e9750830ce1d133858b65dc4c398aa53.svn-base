...
public class NettyWorkerClientRequestProcessor<I extends WritableComparable,
  ...
  public NettyWorkerClientRequestProcessor(
   ...
   if("page".equals(configuration.get("giraph.engine", "default"))) 
	   this.asyncLocal = true;
   this.isOpen = false;
   
   LOG.info("aysn local="+this.asyncLocal);
  ...
  public boolean sendMessageRequest(I destVertexId, M message) {
    ...
    /**
     * NOTE: here simply count the remote message before it is merged.
     */
  ...
  private void doRequest(WorkerInfo workerInfo,
    ...
    if (serviceWorker.getWorkerInfo().getTaskId() ==
        ...
        this.localtime += System.currentTimeMillis() - start_time;
//      ((WorkerRequest) writableRequest).doRequest(serverData);
      localRequests.inc();
    } else {
      ...
      /* 
       * It is incorrect that counting remote message here, because
       * the single messages have been merged. 
       * */
//      this.remoteMessage++;
    }
  }
  ...
  @Override
	public long getLocalRequestCost() {
		  long result = this.localtime;
		  this.localtime = 0;
		  return result;
	}
  
  public long getRemoteMessage(){
	  long result = this.remoteMessage;
	  this.remoteMessage = 0;
	  return result;
  }
