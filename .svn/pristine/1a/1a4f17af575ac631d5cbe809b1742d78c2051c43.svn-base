...
import org.apache.giraph.subgraph.graphextraction.GraphExtraction;
import org.apache.giraph.subgraph.graphextraction.GraphExtractionWithArrayBasedStore;
 ...
 /**
  ...
  private void instantiateBspService(String serverPortList,
    ...
    if (graphFunctions.isWorker()) {  	
    	if("page".equals(conf.get("giraph.engine","default"))){
  	      if (LOG.isInfoEnabled()) {
  	        LOG.info("setup: Starting up PAGE ...");
  	      }   
    ...
    CallableFactory<Collection<PartitionStats>> callableFactory =
        new CallableFactory<Collection<PartitionStats>>() {
          ...
          public Callable<Collection<PartitionStats>> newCallable(
              int callableId) {
        	  if(GiraphConstants.GRAPHOLAP_PATH_EVALUATION_IMPR_NORMAL.get(conf)) {
        	  ...
        	  else if(GiraphConstants.GRAPHOLAP_PATH_EVALUATION_IMPR_ARRAY.get(conf)) {
          		return new GraphExtractionWithArrayBasedStore(
    					context,
    					graphState,
    					messageStore,
    					computePartitionIdQueue,
  ...
  public boolean isSamePartition(int vid1, int vid2) {
	return this.serviceWorker.isSamePartition(vid1, vid2);
  }

  /**
   * retrieve current executing queries based on 
   * the superstep number.
   * 
   * @return
   */
  public ArrayList<QueryNode> getQueries() {
	return serviceWorker.getQueries();
  }

  public boolean isLeftQuery(int qid) {
	return serviceWorker.isLeftQuery(qid);
  }

  public QueryNode getQueryByChild(int qid) {
	return serviceWorker.getQueryByChild(qid);
  }

  public ArrayList<RawQueryNode> getLinePattern() {
	return serviceMaster.getLinePattern();
  }
  
  public void setQueryPlan(QueryPlan qp) {
	  this.serviceMaster.setQueryPlan(qp);
  }
  
  public int getPlanSize() {
	  return serviceWorker.getPlanSize();
  }
}
