...
import org.apache.giraph.tools.utils.StringWritable;
...
public class StringWritableTester extends  
Vertex<IntWritable, IntWritable, NullWritable, StringWritable> {
	...
	@Override
	public void compute(Iterable<StringWritable> messages) throws IOException {
		if(!visited) {
			if(messages.iterator().hasNext() || isSource()){
				visited = true;
				sendMessageToAllEdges(new StringWritable("123"));
			}
		}
		voteToHalt();
	}
	...
	/** Vertex InputFormat */
	public static class StringWritableTesterInputFormat extends
		AdjacencyListTextVertexInputFormat<IntWritable, IntWritable, NullWritable, StringWritable> {
	...
	/**
	 * Simple VertexOutputFormat that supports {@link StringWritableTester}
	 */
	public static class StringWritableTesterOutputFormat extends
		TextVertexOutputFormat<IntWritable, IntWritable, NullWritable> {
			...
			/**
			 * Simple VertexWriter that supports {@link StringWritableTester}
			 */
