...
//  private final RemoteTimeTracker remoteTimeTracker = new RemoteTimeTracker();
//  private final RequestDecoder requestDecoder;
  private final CustomizedBlockingQueue<Runnable> cbq = new CustomizedBlockingQueue<Runnable>();
  /** Send buffer size */
...
//    requestDecoder =  new RequestDecoder(conf, byteCounter);
  }
...
//          pipeline.addLast("remoteTimeTracker", remoteTimeTracker);
         
          /* add exeution handler here */
...
//            pipeline.addAfter("requestDecoder",
//                    "executionHandler", executionHandler);
          }
          return pipeline;
/*if_not[HADOOP_NON_SECURE]*/
...
/*end[HADOOP_NON_SECURE]*/
  ...
  public void setNettyServerExecutionThreadslSize(int nrmp){
	  if (conf.getBoolean(
		        GiraphConstants.NETTY_SERVER_USE_EXECUTION_HANDLER,
		        GiraphConstants.NETTY_SERVER_USE_EXECUTION_HANDLER_DEFAULT)) {
	      int executionThreads = nrmp;
//	      LOG.info("Before: max="+((ThreadPoolExecutor)executionHandler.getExecutor()).getMaximumPoolSize()+
//	    		  " core="+((ThreadPoolExecutor)executionHandler.getExecutor()).getCorePoolSize());
	      if(((ThreadPoolExecutor)executionHandler.getExecutor()).getMaximumPoolSize() < nrmp){
	    	  ((ThreadPoolExecutor)executionHandler.getExecutor()).setMaximumPoolSize(nrmp);
	      }
	      ((ThreadPoolExecutor)executionHandler.getExecutor()).setCorePoolSize(nrmp);
	      if (LOG.isInfoEnabled()){
//	        LOG.info("Set rmp: " + executionThreads + ".");
            LOG.info("After: max="+((ThreadPoolExecutor)executionHandler.getExecutor()).getMaximumPoolSize()+
		    		  " core="+((ThreadPoolExecutor)executionHandler.getExecutor()).getCorePoolSize());
	      }
	    } else {
	    	LOG.info("Failed to set size, because of no ExecutionHandler used.");
	    }
  }
  
  public void resetRemoteTimeTrack(){
//	  remoteTimeTracker.resetTime();
//	  requestDecoder.resetTime();
//	  cbq.resetTime();
	  cbq.resetTime();
  }
  
  public long getRemoteTimeInterval(){
//	  return remoteTimeTracker.getTimeInterval();
//	  return requestDecoder.getTimeInterval();
//	  return cbq.getTimeInterval();
	  return cbq.getBlockingTime();
  }
