...
public class HashMasterPartitioner<I extends WritableComparable,
  ...
  public Collection<PartitionOwner> createInitialPartitionOwners(
    ...
    /* print <partition id, mapid> mapping */
    for(int idx = 0; idx < partitionOwnerList.size(); idx++){
    	System.out.println("host:"+partitionOwnerList.get(idx).getWorkerInfo().getHostnameId()+ " ==> pid="+idx);
    }
    return ownerList;
  }
