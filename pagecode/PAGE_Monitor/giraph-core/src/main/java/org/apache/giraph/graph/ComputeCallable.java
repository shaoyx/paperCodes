      ...
      if(LOG.isInfoEnabled()){
 	     LOG.info("\n\tlocalRequestCost="+workerClientRequestProcessor.getLocalRequestCost()+
 	    		 "\n\tlocalRequest="+workerClientRequestProcessor.getLocalCounter()+  
	        		...
	        		"\n\trealCompTime="+realCompTime+
	        		"\n\tsendTime="+sendTime+
	        		"\n\tedgecut="+graphState.getGraphTaskManager().getEdgeCuts()+
    ...
    synchronized (partition) {
      ...
      for (Vertex<I, V, E, M> vertex : partition) {
        ...
        if (!vertex.isHalted()) {
          ...
          try {
        	vertex.resetSendTime();
        	long start_time = System.currentTimeMillis();
        	vertex.resetMsgCount();
            vertex.compute(messages);
            msgCount += vertex.getMsgCount();
            realCompTime += System.currentTimeMillis() - start_time;
            sendTime += vertex.getSendTime();
          } finally {
