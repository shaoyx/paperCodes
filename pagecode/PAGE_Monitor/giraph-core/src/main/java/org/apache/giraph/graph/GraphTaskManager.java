...
import org.apache.giraph.worker.BspServiceWorkerPageWithQueue;
import org.apache.giraph.worker.BspServiceWorkerWithAsyncLocal;
import org.apache.giraph.worker.BspServiceWorkerWithBlockQueue;
import org.apache.giraph.worker.BspServiceWorkerWithBlockQueueThread;
...
public class GraphTaskManager<I extends WritableComparable, V extends Writable,
  ...
  public GraphTaskManager(Mapper<?, ?, ?, ?>.Context context) {
    ...
    this.monitor = new Monitor(context.getConfiguration());
//    context.getc
  }
...
//      System.err.println("Superstep="+superstep);
      ...
      serviceWorker.resetRemoteTime();
      serviceWorker.setStart();
      if (numPartitions > 0) {
        processGraphPartitions(context, partitionStatsList, graphState,
          messageStore, numPartitions, numThreads);
      }
      
      serviceWorker.setFinishTime();
      
      finishedSuperstepStats = completeSuperstepAndCollectStats(
        partitionStatsList, superstepTimerContext, graphState);
    	LOG.info("Superstep = "+superstep +" MemoryUsed="+ 
    			  (Runtime.getRuntime().totalMemory() - Runtime.getRuntime().freeMemory())/1024.0/1024.0);
      // END of superstep compute loop
    	
      monitor.setSuperstepTime(superstep, superstepTimer.getLong());
      /* calculate metrics */
      serviceWorker.stopThreads(monitor);
      LOG.info(monitor.toString());
//      double sp = serviceWorker.getSp();
//      double sl = serviceWorker.getSl();
//      double sr = serviceWorker.getSr();
//      LOG.info("\nSp : " + sp + "\nSl : " + sl + "\nSr : " + sr);
//      LOG.info("GenerationTime: " + serviceWorker.getRunTime());
//      LOG.info("Idea Processor: "+ Math.round(((sl+sr)/sp+0.5))
//    		  +"\nlocal Processor: "+Math.round((sl/sp+0.5))
//    		  +"\nremote Processor: "+Math.round((sr/sp+0.5)));
//      monitor.setSLMetrics(serviceWorker.get, time);
//      monitor.setSRMetrics(msgNum, serviceWorker.getRemoteFinishTime() - serviceWorker.getRemoteStartTime());
//      monitor.setSPMetrics(, time);
      serviceWorker.restartThreads(monitor); 
  ...
  private void instantiateBspService(String serverPortList,
    ...
    if (graphFunctions.isWorker()) {
      if (LOG.isInfoEnabled()) {
        ...
        LOG.info("Engine info: "+ this.conf.get(GiraphConstants.GIRAPH_ENGINE,GiraphConstants.DEFAULT_GIRAPH_ENGINE));
      }
      
      
      /**
       * Create specific giraph engine according configuration.
       */
      if("queue".equals(this.conf.get(GiraphConstants.GIRAPH_ENGINE,GiraphConstants.DEFAULT_GIRAPH_ENGINE))){
          if (LOG.isInfoEnabled()) {
            LOG.info("instantiate queue Engine!");
          }
    	  serviceWorker = new BspServiceWorkerWithBlockQueue<I, V, E, M>(
        serverPortList,
        sessionMsecTimeout,
        context,
        this);
      ...
      //ssss/
      else if("queuethread".equals(this.conf.get(GiraphConstants.GIRAPH_ENGINE,GiraphConstants.DEFAULT_GIRAPH_ENGINE))){
          if (LOG.isInfoEnabled()) {
              LOG.info("instantiate queuethread Engine!");
            }
    	  serviceWorker = new BspServiceWorkerWithBlockQueueThread<I, V, E, M>(
    		        serverPortList,
    		        sessionMsecTimeout,
    		        context,
    		        this);
      }
      else if("asynclocal".equals(this.conf.get(GiraphConstants.GIRAPH_ENGINE,GiraphConstants.DEFAULT_GIRAPH_ENGINE))){
          if (LOG.isInfoEnabled()) {
              LOG.info("instantiate asynclocal Engine!");
            }
    	  serviceWorker = new BspServiceWorkerWithAsyncLocal<I, V, E, M>(
    		        serverPortList,
    		        sessionMsecTimeout,
    		        context,
    		        this);
      }
      else if("pagewithqueue".equals(this.conf.get(GiraphConstants.GIRAPH_ENGINE,GiraphConstants.DEFAULT_GIRAPH_ENGINE))){
          if (LOG.isInfoEnabled()) {
              LOG.info("instantiate pagewithqueue Engine!");
            }
    	  serviceWorker = new BspServiceWorkerPageWithQueue<I, V, E, M>(
    		        serverPortList,
    		        sessionMsecTimeout,
    		        context,
    		        this);
      }
      else if("default".equals(this.conf.get(GiraphConstants.GIRAPH_ENGINE,GiraphConstants.DEFAULT_GIRAPH_ENGINE))){
          if (LOG.isInfoEnabled()) {
            LOG.info("instantiate default Engine!");
          }
          ...
          if (LOG.isInfoEnabled()) {
            LOG.info("Invalid Parameter: + "+this.conf.get(GiraphConstants.GIRAPH_ENGINE,GiraphConstants.DEFAULT_GIRAPH_ENGINE)+"for Giraph Engine!");
          }
      }
      if (LOG.isInfoEnabled()) {
...
public double getEdgeCuts() {
	...
	return serviceWorker.getEdgeCuts();
}
}
