...
public class UnsafeByteArrayOutputStream extends OutputStream
  ...
  public void writeUTF(String s) throws IOException {
    ...
    ensureSize(strlen+2);
    /*XXX: use the little-endian order here. */
    buf[pos++] = (byte) ((utflen >>> 0) & 0xFF);
    buf[pos++] = (byte) ((utflen >>> 8) & 0xFF);

//    System.out.println("In Write s="+s+": the len =" + strlen +" utflen="+utflen+"");
//    System.out.println("byte1="+buf[pos-2]+" byte2="+buf[pos-1]+" end pos"+pos);
    int i = 0;
    for (i = 0; i < strlen; i++) {
      c = s.charAt(i);
      if (!((c >= 0x0001) && (c <= 0x007F))) {
