...
import org.apache.giraph.partition.IdenticalWithRelabelWorkerPartitioner;
...
public class VertexInputSplitsCallable<I extends WritableComparable,
  ...
  private final IBspServiceWorker<I, V, E, M> bspServiceWorker;
  ...
  public VertexInputSplitsCallable(
      ...
      IBspServiceWorker<I, V, E, M> bspServiceWorker,
  ...
  protected VertexEdgeCount readInputSplit(
    ...
    while (vertexReader.nextVertex()) {
      ...
      WorkerGraphPartitioner wgp = bspServiceWorker.getWorkerGraphPartitioner();
      /**
       * if wgp is IndenticalWithRelabelWorkerPartitioner, then recreate a vertex
       * with new label
       */
      if(wgp instanceof IdenticalWithRelabelWorkerPartitioner){
    	  readerVertex = ((IdenticalWithRelabelWorkerPartitioner) wgp).createNewVeretex(readerVertex);
//    	  int newVertexId = ((IdenticalWithRelabelWorkerPartitioner) wgp).getNewVertexId(readerVertex.getId());
//    	  readerVertex.setId(new IntWritable(newVertexId));
//    	  for(Edge<I,E> edge : readerVertex.getEdges()){
//    		if(edge instanceof DefaultEdge){
//    			newVertexId = ((IdenticalWithRelabelWorkerPartitioner) wgp).getNewVertexId(edge.getTargetVertexId());
//    			if(LOG.isDebugEnabled()){
//    				LOG.debug("Change Edge: oldTaregt="+edge.getTargetVertexId().hashCode()+" newTarget="+newVertexId);
//    			}
//    			((DefaultEdge) edge).setTargetVertexId(new IntWritable(newVertexId));
//    		}  
//    	  }
      }
      ...
      if(wgp instanceof IterativeStreamWorkerPartitioner){
    	  ((IterativeStreamWorkerPartitioner) wgp).determinePartitionId(readerVertex);
      }
      ...
      PartitionOwner partitionOwner =
		...
		if(LOG.isDebugEnabled()){
			LOG.debug("After determine the partitionOwner: newVid="+readerVertex.getId().hashCode());
	    	  for(Edge<I,E> edge : readerVertex.getEdges()){
	    		  LOG.debug("Edge TargetId="+edge.getTargetVertexId().hashCode());
	    	  }
		}
